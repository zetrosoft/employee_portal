	...
	if employees:
		...
		if not cleaned_employee_cell_number or len(cleaned_employee_cell_number) < 5:
			frappe.log_error(
				...
				message=f"Employee {employee.name} cleaned cell_number is too short or missing.",
			)
		...
		if pwd == expected_pwd:
			...
			if frappe_user_name:
				...
				user_doc.first_name = (
					employee.employee_name.split(" ")[0] if employee.employee_name else employee.name
				)
				user_doc.last_name = (
					" ".join(employee.employee_name.split(" ")[1:])
					if employee.employee_name and len(employee.employee_name.split(" ")) > 1
					else ""
				)
				user_doc.user_type = (
					"Website User"  # Or "System User" if desk access is needed for all features
				)
			...
			if not user_doc.has_role("Employee User"):
				...
				frappe.log_error(
					title="Custom Login Info", message=f"Assigned 'Employee User' role to {user_doc.name}."
				)

			# Link Frappe User to Employee
			if employee.user_id != user_doc.name:
				frappe.db.set_value(
					"Employee", employee.name, "user_id", user_doc.name, update_modified=False
				)
				frappe.db.commit()  # Commit this change immediately
				frappe.log_error(
					title="Custom Login Info",
					message=f"Linked Frappe User {user_doc.name} to Employee {employee.name}.",
				)
			...
			frappe.db.commit()
			frappe.log_error(
				title="Custom Login Success",
				message=f"Employee {employee.name} successfully logged in as Frappe User: {user_doc.name}.",
			)
			return  # Successfully handled login

		else:  # Password invalid
			frappe.log_error(
				title="Custom Login Failed", message=f"Invalid password for employee {employee.name}."
			)
		...
		frappe.log_error(
			title="Custom Login Fallback",
			message=f"No unprovisioned employee found with cell_number {cleaned_user_cell_number}. Falling back to standard Frappe login.",
		)
