			...
			if line.strip().startswith("INSERT INTO `tabEmployee` VALUES"):
				...
				if match:
					...
					try:
						...
						)  # 'employee' field, usually the NIK
...
def create_employee_users():
	...
	for data in employees_data:
		...
		if not employee_id:
			frappe.log_error(
				f"Skipping employee due to missing employee_id: {employee_name}", "Create Employee User"
			)
			continue
		...
		if not formatted_cell_number:
			frappe.log_error(
				f"Skipping employee {employee_name} due to invalid cell number: {cell_number}",
		...
		if not password:
			frappe.log_error(
				f"Skipping employee {employee_name} due to invalid date of birth for password: {date_of_birth_str}",
				"Create Employee User",
			)
			continue
		...
		if not employee_doc:
			frappe.log_error(
				f"Could not find Employee DocType for {employee_name} ({employee_id})", "Create Employee User"
			)
			continue
		...
		if not employee_doc.user_id:
			...
			if frappe.db.exists("User", user_id):
				...
				try:
					user = frappe.get_doc(
						{
							...
							"last_name": " ".join(employee_name.split(" ")[1:])
							if employee_name and len(employee_name.split(" ")) > 1
							else "",
							"enabled": 1,
					...
					frappe.log_error(
						f"Error creating user {user_id} for {employee_name}: {e}", "Create Employee User"
					)
					continue
			...
			if frappe.db.exists("User", user_id_from_doc):
				...
				frappe.log_error(
					f"User {user_id_from_doc} linked in Employee DocType for {employee_name} does not exist.",
					"Create Employee User",
				)
