...
def main():
	...
	if not frappe.db.exists("DocType", TARGET_DOCTYPE):
		...
		if not frappe.db.exists("DocType", TARGET_DOCTYPE):
			frappe.throw(
				f"DocType '{TARGET_DOCTYPE}' atau 'Custom DocPerm' tidak ditemukan. Skrip tidak bisa dilanjutkan."
			)
	...
	for perm_def in permissions_data:
		...
		rp_name = frappe.db.get_value(
			TARGET_DOCTYPE, {"role": role_name, "parent": target_doctype, "permlevel": permlevel}, "name"
		)
		...
		try:
			...
			frappe.log_error(
				title="Setup Permissions Error",
				message=f"Gagal {action} {TARGET_DOCTYPE} untuk {target_doctype}: {e}",
			)
			frappe.db.rollback()
