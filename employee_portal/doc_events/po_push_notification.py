...
def execute(doc, method):
    ...
    """
    frappe.log_error(title="[PO Notification Debug]", message=f"--- Script execute dipanggil untuk {doc.name} ---")
    ...
    try:
        ...
        if doc.workflow_state in pending_states:
            ...
            if approvers:
                ...
                frappe.log_error(title="[PO Notification Debug]", message=f"Notifikasi persetujuan untuk {doc.name} dikirim ke role {approver_role}.")
            else:
        ...
        # --- KONDISI 2: Dokumen ditolak ---
            ...
            if user_to_notify:
                ...
                notification_content = f"Purchase Order {doc.name} telah Ditolak. Status dokumen sekarang Dibatalkan."
                ...
                frappe.log_error(title="[PO Notification Debug]", message=f"Notifikasi penolakan untuk {doc.name} dikirim ke {user_to_notify} dan docstatus di set ke 2.")
        ...
        # --- KONDISI 3: Dokumen telah disetujui (Submitted) ---
            ...
            if user_to_notify:
                ...
                frappe.get_doc(notification_log).insert(ignore_permissions=True)
                publish_realtime('notification', user=user_to_notify)
                frappe.log_error(title="[PO Notification Debug]", message=f"Notifikasi 'Submitted' untuk {doc.name} dikirim ke {user_to_notify}.")

    except Exception:
        frappe.log_error(
            title='Gagal Menjalankan Hook Notifikasi Purchase Order',
