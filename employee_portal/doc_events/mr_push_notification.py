...
def execute(doc, method):
    ...
    frappe.log_error(
        ...
        message=f"Hook '{method}' triggered for MR {doc.name}. Status: {doc.docstatus}, Workflow State: '{doc.workflow_state}'"
    )
    ...
    try:
        ...
        if method == "on_update":
            ...
            if doc.workflow_state in pending_states:
                ...
                approver_role = frappe.get_value(
                    "Workflow Transition",
                    {"parent": "Material Request Approval", "state": doc.workflow_state, "action": current_action},
                    "allowed"
                )
                ...
                if approvers:
                    ...
                    for user_id in approvers:
                        notification_log = {
                            ...
                            "document_name": doc.name, "subject": notification_title, "for_user": user_id,
            ...
            # --- KONDISI 2: Dokumen ditolak ---
                ...
                if user_to_notify:
                    ...
                    notification_log = {
                        "doctype": "Notification Log", "type": "Alert", "document_type": doc.doctype,
                        "document_name": doc.name, "subject": notification_title, "for_user": user_to_notify,
                        "email_content": notification_content
                    }
                    frappe.get_doc(notification_log).insert(ignore_permissions=True)
        ...
        # Logic for final submission
            ...
            frappe.log_error(title="[MR Notification Debug]", message=f"Owner '{doc.owner}' ditambahkan ke daftar notifikasi.")
            ...
            for role in purchasing_roles:
                ...
                frappe.log_error(title="[MR Notification Debug]", message=f"Query untuk role '{role}' menemukan {user_count} user.")
                
                for row in users_in_role_tuple:
                    users_to_notify.add(row[0])

            frappe.log_error(title="[MR Notification Debug]", message=f"Daftar final user untuk dinotifikasi: {list(users_to_notify)}")
