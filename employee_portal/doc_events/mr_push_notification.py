	...
	try:
			...
			# --- KONDISI 2: Dokumen ditolak ---
				...
				if user_to_notify:
					...
					frappe.log_error(
						title="[MR Notification Debug]",
						message=(
							f"Notifikasi penolakan untuk {doc.name} "
		...
		# Logic for final submission
			...
			if users_to_notify:
				...
				notification_content = (
					f"Material Request {doc.name} telah disetujui dan siap untuk proses selanjutnya."
				)
				...
				frappe.log_error(
					title="[MR Notification Debug]",
					message=(f"Notifikasi 'Submitted' untuk {doc.name} dikirim ke: {user_list_str}."),
				)

	except Exception:
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Material Request", message=frappe.get_traceback()
		)
