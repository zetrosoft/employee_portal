...
def execute(doc, method):
	...
	try:
		...
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Delivery Note", message=frappe.get_traceback()
		)
...
def notify(doc, roles=None, users=None, subject="", content=""):
	...
	if not recipients:
		frappe.log_error(
			title="[DN Notification]", message=f"No users found for roles {roles} to notify for {doc.name}."
		)
		return
	...
	frappe.log_error(
		title="[DN Notification]", message=f"Notification '{subject}' sent to {len(recipients)} users."
	)
