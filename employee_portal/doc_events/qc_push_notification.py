...
def execute(doc, method):
	frappe.log_error(
		title="[QC Notification Debug]", message=f"--- Script execute dipanggil untuk {doc.name} ---"
	)
	...
	try:
		...
		if doc.workflow_state == "Pending QAM Approval":
			...
			frappe.log_error(
				title="[QC Notification Debug]", message=f"Nilai dari approver_role: {approver_role}"
			)
		...
		# --- KONDISI 2: Dokumen ditolak (Cancelled) ---
			...
			if user_to_notify:
				...
				notification_content = (
					f"Quality Inspection {doc.name} telah Ditolak. Status dokumen sekarang Dibatalkan."
				)
		...
		# --- KONDISI 3: Dokumen telah disetujui (Submitted) ---
			...
			if user_to_notify:
				...
				frappe.log_error(
					title="[QC Notification Debug]",
					message=f"Notifikasi 'Submitted' untuk {doc.name} dikirim ke {user_to_notify} dan docstatus di set ke 1.",
				)

	except Exception:
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Quality Inspection", message=frappe.get_traceback()
		)
