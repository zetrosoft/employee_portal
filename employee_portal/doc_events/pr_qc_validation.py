...
def validate_on_submit(doc):
    ...
    """
    items_to_check = [item for item in doc.items if frappe.db.get_value("Item", item.item_code, "item_group") == "Persediaan Bahan Baku"]
        ...
        if not qi:
            ...
            for reading in qi_doc.readings:
                if reading.status == "Rejected":
                    ...
                    parameter_description = frappe.get_cached_value('Quality Inspection Parameter', reading.specification, 'description')
            ...
            if rejected_reasons_list:
                ...
                result["keterangan"] = "Item ditolak karena salah satu atau lebih kriteria inspeksi tidak terpenuhi (detail tidak tersedia)."
    ...
    frappe.db.set_value("Purchase Receipt", doc.name, "custom_qc_validation_summary", html_summary, update_modified=False)
    ...
    if has_errors:
        ...
        frappe.db.set_value("Purchase Receipt", doc.name, "custom_qc_validation_summary", "", update_modified=False)
