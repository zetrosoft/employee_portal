...
def execute(doc, method):
	...
	try:
		...
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Purchase Invoice", message=frappe.get_traceback()
		)
...
def notify(doc, roles=None, users=None, subject="", content=""):
	...
	if not roles and not users:
		frappe.log_error(
			...
			message=f"Notification for {doc.name} aborted: No recipients specified.",
		)
		return
	...
	if not recipients:
		frappe.log_error(
			title="[PI Notification]", message=f"No users found for roles {roles} to notify for {doc.name}."
		)
		return

	for user in recipients:
		# Gunakan new_doc agar metadata dokumen selalu baru di setiap iterasi
	...
	frappe.log_error(
		title="[PI Notification]", message=f"Notification '{subject}' sent to {len(recipients)} users."
	)
