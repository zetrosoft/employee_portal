...
def send_notification_on_submit(doc, method):
	...
	"""
	frappe.log_error(
		title="[QI Notif Debug]", message=f"Fungsi send_notification_on_submit dipanggil untuk QI: {doc.name}"
	)

	if not doc.reference_name:
		frappe.log_error(
			title="[QI Notif Debug]", message=f"QI {doc.name}: reference_name kosong. Notifikasi dibatalkan."
		)
		return
	...
	colored_status = (
		f"<b style='color:green;'>{inspection_status}</b>"
		if inspection_status == "Accepted"
		else f"<b style='color:red;'>{inspection_status}</b>"
	)
	...
	if users_to_notify_list:
		...
		send_notification(
			users_to_notify_list, notification_subject, notification_content, linked_doctype, linked_docname
		)
		frappe.log_error(
			title="[QI Notif]",
			message=f"Notifikasi hasil QI {doc.name} dikirim ke {len(users_to_notify_list)} user ({users_to_notify_list}) untuk {doc.reference_type} {doc.reference_name}.",
		)
	else:
		frappe.log_error(
			title="[QI Notif]",
			message=f"QI {doc.name}: Tidak ada user yang ditemukan (owner atau peran umum) untuk dinotifikasi pada {doc.reference_type} {doc.reference_name}. Notifikasi dibatalkan.",
		)
