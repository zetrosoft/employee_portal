...
def execute(doc, method):
	frappe.log_error(
		title="[SO Notification Debug]",
		message=f"--- Script so_push_notification dipanggil untuk {doc.name} ---",
	)
	...
	try:
		...
		if doc.workflow_state == "Pending SAM Approval":
			...
			approvers_tuple = frappe.db.sql(
				"SELECT T1.parent FROM `tabHas Role` AS T1 JOIN `tabUser` AS T2 ON T1.parent = T2.name WHERE T1.role = %s AND T2.enabled = 1",
				(approver_role,),
			)
			...
			frappe.log_error(
				title="[SO Notification Debug]", message=f"Notifikasi dikirim ke role {approver_role}."
			)
		elif doc.workflow_state == "Pending Director Approval":
			frappe.log_error(
				title="[SO Notification Debug]", message="Masuk kondisi: 'Pending Director Approval'"
			)
		...
		# --- KONDISI 3: Dokumen Ditolak ---
			...
			sales_managers = [
				...
				for row in frappe.db.sql(
					"SELECT T1.parent FROM `tabHas Role` AS T1 JOIN `tabUser` AS T2 ON T1.parent = T2.name WHERE T1.role = 'Sales Manager' AND T2.enabled = 1"
				)
			]
		...
		# --- KONDISI 4: Dokumen Disetujui (Submitted) ---
			...
			logistic_users = [
				row[0]
				for row in frappe.db.sql(
					"SELECT T1.parent FROM `tabHas Role` AS T1 JOIN `tabUser` AS T2 ON T1.parent = T2.name WHERE T1.role = 'Logistics User' AND T2.enabled = 1"
				)
			]
			logistic_managers = [
				row[0]
				for row in frappe.db.sql(
					"SELECT T1.parent FROM `tabHas Role` AS T1 JOIN `tabUser` AS T2 ON T1.parent = T2.name WHERE T1.role = 'Logistics Manager' AND T2.enabled = 1"
				)
			]
		...
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Sales Order", message=frappe.get_traceback()
		)
