...
def execute(doc, method):
	frappe.log_error(
		title="[PE Notification Debug]", message=f"--- Script execute dipanggil untuk {doc.name} ---"
	)
	...
	try:
		...
		if doc.workflow_state == "Pending AM Approval":
			...
			frappe.log_error(
				title="[PE Notification Debug]", message=f"Nilai dari approver_role: {approver_role}"
			)
		...
		# --- KONDISI 3: Dokumen ditolak ---
			...
			accounts_managers = [
				...
				for row in frappe.db.sql(
					"SELECT T1.parent FROM `tabHas Role` AS T1 JOIN `tabUser` AS T2 ON T1.parent = T2.name WHERE T1.role = 'Accounts Manager' AND T2.enabled = 1"
				)
			]
			...
			if recipients:
				...
				notification_content = (
					f"Payment Entry {doc.name} telah Ditolak. Status dokumen sekarang Dibatalkan."
				)
				...
				frappe.log_error(
					title="[PE Notification Debug]",
					message=f"Notifikasi penolakan untuk {doc.name} dikirim ke {recipients} dan docstatus di set ke 2.",
				)
		...
		# --- KONDISI 4: Dokumen telah disetujui (Submitted) ---
			...
			if recipients:
				...
				frappe.log_error(
					title="[PE Notification Debug]",
					message=f"Notifikasi 'Submitted' untuk {doc.name} dikirim ke {recipients}.",
				)

	except Exception:
		frappe.log_error(
			title="Gagal Menjalankan Hook Notifikasi Payment Entry", message=frappe.get_traceback()
		)
