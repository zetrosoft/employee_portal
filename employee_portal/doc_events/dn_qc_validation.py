...
def execute_validation(doc, method):
	...
	if not items_to_check:
		...
		frappe.db.set_value(
			"Delivery Note", doc.name, "custom_qc_validation_summary", "", update_modified=False
		)
		return
	...
	frappe.db.set_value(
		"Delivery Note", doc.name, "custom_qc_validation_summary", html_summary, update_modified=False
	)
...
def update_delivery_note_on_qi_submit(doc, method):
	...
	frappe.log_error(
		title="[DN QC Debug]",
		message=f"update_delivery_note_on_qi_submit dipanggil untuk QI: {doc.name}, Status QI: {doc.status}, Ref Type: {doc.reference_type}, Ref Name: {doc.reference_name}",
	)
	...
	if not (doc.reference_type == "Delivery Note" and doc.reference_name):
		frappe.log_error(
			title="[DN QC Debug]",
			message=f"QI {doc.name}: Bukan untuk Delivery Note atau reference_name kosong. Keluar.",
		)
		return
	...
	try:
		...
		frappe.log_error(
			title="[DN QC Debug]",
			message=f"QI {doc.name}: DN {dn_doc.name} ditemukan. Current DN state: {dn_doc.workflow_state}",
		)
		...
		if dn_doc.workflow_state != "Pending QC Inspection":
			frappe.log_error(
				title="[DN QC Debug]",
				message=f"QI {doc.name}: DN {dn_doc.name} tidak dalam status 'Pending QC Inspection'. Keluar.",
			)
			return
		...
		if new_state:
			...
			try:
				...
				frappe.throw(
					_(
						"Gagal mengubah status Delivery Note. QI tidak disubmit. Mohon periksa Error Log untuk detail."
					)
				)
			finally:
		...
		frappe.log_error(
			title="Hook QI Gagal", message=f"Dokumen Delivery Note {doc.reference_name} tidak ditemukan."
		)
		...
		frappe.throw(
			_(
				"Terjadi error umum saat memperbarui Delivery Note. QI tidak disubmit. Mohon periksa Error Log untuk detail."
			)
		)
